...
void RTLSClient::openLogFile(QString)
{
...
//update the history array and the average
{
    ...
    rp.count++;
    //qDebug() << rp.x_arr[idx] << rp.y_arr[idx] << rp.z_arr[idx];
...
void RTLSClient::processTagRangeReport(int aid, int tid, int range, int lnum, int seq)
{
    ...
    //////////////////////////
//    qDebug() << idx << "  " << rp.rangeSeq << rp.averr_x << rp.averr_y << rp.averr_z;
//    qDebug() << rp.avsq_x << rp.avsq_y << rp.avsq_z;
//    qDebug() << rp.av_x << rp.av_y << rp.av_z;
//    qDebug() << rp.fx << rp.fy << rp.fz;
//    qDebug() << rp.r95;
//    qDebug() << rp.rangeValue[seq][0] << rp.rangeValue[seq][1] << rp.rangeValue[seq][2] << rp.rangeValue[seq][3];
//    qDebug() << "------------------------------------------------------------";
    //update the list entry
...
void RTLSClient::trilaterateTag(int tid, int seq)
{
    ...
    if(count >= 3)
    {
        qDebug() << "try to get location" << iii++;
