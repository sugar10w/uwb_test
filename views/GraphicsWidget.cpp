...
static SocketClient udp_socket(8887, "192.168.1.1");
 ...
 *
 * */
void GraphicsWidget::tagPos(quint64 tagId = 0, double x = 1232, double y = 243, double z = 531)
{
    pos_xyz_info dobbyPos;
    pos_xyz_info dobbyTarget;
    pos_info dobbyPosition;
    pos_xyz_info *pBuffer_pos;
    pos_xyz_info *pBuffer_target;
    if ((x > 0.0001) && (y > 0.0001))
    {
        dobbyPos.x = x;
        dobbyPos.y = y;
        dobbyPos.z = z;
        dobbyPos.writeflag = 1;
    }
    else
    {
        dobbyPos.x = x;
        dobbyPos.y = y;
        dobbyPos.z = z;
        dobbyPos.writeflag = 0;
    }
    pBuffer_pos = MapViewOfFile(hMap_pos, FILE_MAP_ALL_ACCESS, 0, 0, 0);
    pBuffer_target = MapViewOfFile(hMap_target, FILE_MAP_ALL_ACCESS,0 ,0, 0);
    qDebug()<<"Share Memory ready";
    if (NULL == pBuffer_pos || NULL == pBuffer_target)
    {
        qDebug() << "Share Memory Error\n" << endl;
    }
    if (dobbyPos.writeflag)
    {
        memcpy(pBuffer_pos, &dobbyPos, sizeof(pos_xyz_info));
        dobbyPosition.x = dobbyPos.x;
        dobbyPosition.y = dobbyPos.y;
        dobbyPosition.z = dobbyPos.z;
        qDebug() << "send Pos"<< dobbyPos.x;
    }
    if ((pos_xyz_info*)pBuffer_target->writeflag)
    {
        memcpy(&dobbyTarget, pBuffer_target, sizeof(pos_xyz_info));
        pBuffer_target->writeflag = 0;
        dobbyPosition.targetx = dobbyTarget.x;
        dobbyPosition.targety = dobbyTarget.y;
        dobbyPosition.targetz = dobbyTarget.z;
        qDebug() << "recv Target "<< dobbyPosition.targetx;
    }

    dobbyPosition.writeflag = (dobbyPos.writeflag) && (dobbyPosition.writeflag)
            && (dobbyPosition.targetz>50) && (dobbyPosition.targetz<250);

    byte SendBuffer[200];
    memset(SendBuffer,0,sizeof(SendBuffer));
    memcpy(SendBuffer,&dobbyPosition,sizeof(pos_info));


    //int Ret = send(ClientSocket, SendBuffer, sizeof(pos_info)/*(int)strlen(SendBuffer)*/,0);
    //if ( Ret == SOCKET_ERROR )
    //{
    //    cout<<"Send Info Error::"<<GetLastError()<<endl;
    //}
    udp_socket.send(MyString(SendBuffer, sizeof(pos_info)));
